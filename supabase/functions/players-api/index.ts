...
const SNAPSHOT_MIN_GAP  = 24 * 60 * 60 * 1000;
...
async function handleSave({ session_token, state }: any) {
  ...
  // v25.15.2 — Euros : protection anti-écrasement par save non-sync
  if ("euros" in upd) {
    ...
    const oldEur = Number(old.euros || 0);
    if (oldEur > EUROS_GUARD_THRESHOLD && newEur < oldEur * EUROS_GUARD_RATIO) {
      console.warn("[Save] euros_guard_kept", { maia: old.maia, oldEur, newEur });
      upd.euros = oldEur;
    } else {
      upd.euros = newEur;
    }
  ...
  for (const k of ["epurateur","compresseur","is_connected","tractor_speed_boost"]) {
    if (k in upd) upd[k] = !!upd[k as keyof typeof upd] || !!(old as any)[k];
  }
  ...
  try {
    const { data: lastSnap } = await sb.from("players_snapshots").select("snapshot_at").eq("maia", old.maia).order("snapshot_at", { ascending: false }).limit(1).maybeSingle();
  ...
  } catch (e) { console.warn("[Snapshot] failed (non-blocking)", e); }
...
async function handleLeaderboard({ mode, region, limit }: any) {
  ...
  if (mode === "cumulative_season") {
    let q = sb.from("players").select("maia, username, region, score_network, score_gnv, current_yield, season_started_at, season_start_score_network, season_start_score_gnv, is_connected, digesteurs, epurateur, compresseur, euros").limit(500);
    if (filterRegion) q = q.eq("region", filterRegion);
    const { data, error } = await q;
    if (error) return j({ error: "query_failed", detail: error.message }, 500);
