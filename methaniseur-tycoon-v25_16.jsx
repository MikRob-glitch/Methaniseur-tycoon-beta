...
function Game({ username, region, maia }) {
  ...
  const calcOffline = (sv) => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      // v25.23 — une panne reste active tant que le joueur n'a pas relancé
      // manuellement (l'ancien timer offlineUntil ne déclenche plus d'auto-réparation)
      if (wasOffline) {
    ...
    if (fr > 0) {
      ...
      if (dtStock >= 1) {
        ...
        for (let i = 0; i < 7; i++) {
          ...
          // Une zone offline reste offline jusqu'à relance manuelle (v25.23)
  ...
  useEffect(()=>{
    ...
    const id=setInterval(()=>{
      ...
      for (let i = 0; i < 7; i++) {
        ...
        // Zone offline : reste en panne tant que le joueur n'a pas relancé
        // manuellement via handleRelaunchZone (v25.23 — fix : l'auto-réparation
        // après PANNE_DURATION_MS contredisait la facturation journalière v25.17)
        if (cur === 'offline') {
          continue;
        }
      ...
      <g transform={`translate(${x} ${y})`} style={{opacity: locked ? 0.38 : (isOffline ? 0.55 : 1), transition:"opacity .4s"}}>
        ...
        {isOffline && (
          <g style={{pointerEvents:"all", cursor: relaunchAffordable ? "pointer" : "not-allowed"}}
            ...
            <text y={-15} textAnchor="middle" fontSize="6.5" fill="var(--c-red)" fontWeight={800} letterSpacing=".4">⚠ PANNE · RELANCE REQUISE</text>
